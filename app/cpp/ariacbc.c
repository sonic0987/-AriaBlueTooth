...
Java_com_example_android_bluetoothchat_ARIACBC_processBlock(JNIEnv* env, jobject thiz, jbyteArray inputText, jint nr, jbyteArray rk_, jbyteArray out_,
		jint outputOffset) {
	/*unsigned char *in = (unsigned char *)(*env)->GetPrimitiveArrayCritical(env, inputText, 0);
	unsigned char *rk = (unsigned char *)(*env)->GetPrimitiveArrayCritical(env, rk_, 0);
	unsigned char *out = (unsigned char *)(*env)->GetPrimitiveArrayCritical(env, out_, 0);*/

	jbyte *in = (*env)->GetByteArrayElements(env, inputText, 0);
	jbyte *rk = (*env)->GetByteArrayElements(env, rk_, 0);
	jbyte *out = (*env)->GetByteArrayElements(env, out_, 0);

	//out += outputOffset;

	KISA_ARIA_process_block(in, out+outputOffset, rk, &nr);

	(*env)->ReleaseByteArrayElements(env, inputText, in, 0);
	(*env)->ReleaseByteArrayElements(env, rk_, rk, 0);
	(*env)->ReleaseByteArrayElements(env, out_, out, 0);

	/*(*env)->ReleasePrimitiveArrayCritical(env, inputText, in, 0);
	(*env)->ReleasePrimitiveArrayCritical(env, rk_, rk, 0);
	(*env)->ReleasePrimitiveArrayCritical(env, out_, out, 0);*/
}

JNIEXPORT void JNICALL
Java_com_example_android_bluetoothchat_ARIACBC_ariaEncryptInit(JNIEnv* env, jobject thiz, jbyteArray user_key, jint keyBits, jbyteArray rk_, jintArray nr_) {
	jbyte *userkey = (*env)->GetByteArrayElements(env, user_key, 0);
    jbyte *rk = (*env)->GetByteArrayElements(env, rk_, 0);
	jint *nr = (*env)->GetIntArrayElements(env, nr_, 0);
    ...
    GEN_ROUND_KEY2(rk, wrd0, wrd1, 19);
	GEN_ROUND_KEY2(rk+16, wrd1, wrd2, 19);
	GEN_ROUND_KEY2(rk+32, wrd2, wrd3, 19);
	GEN_ROUND_KEY2(rk+48, wrd3, wrd0, 19);
	GEN_ROUND_KEY2(rk+64, wrd0, wrd1, 31);
	GEN_ROUND_KEY2(rk+80, wrd1, wrd2, 31);
	GEN_ROUND_KEY2(rk+96, wrd2, wrd3, 31);
	GEN_ROUND_KEY2(rk+112, wrd3, wrd0, 31);
	GEN_ROUND_KEY2(rk+128, wrd0, wrd1, 67);
	GEN_ROUND_KEY2(rk+144, wrd1, wrd2, 67);
	GEN_ROUND_KEY2(rk+160, wrd2, wrd3, 67);
	GEN_ROUND_KEY2(rk+176, wrd3, wrd0, 67);
	GEN_ROUND_KEY2(rk+192, wrd0, wrd1, 97);
	if (keyBits > 128) {
		GEN_ROUND_KEY2(rk+208, wrd1, wrd2, 97);
		GEN_ROUND_KEY2(rk+224, wrd2, wrd3, 97);
	}
	if (keyBits > 192) {
		GEN_ROUND_KEY2(rk+240, wrd3, wrd0,  97);
		GEN_ROUND_KEY2(rk+256, wrd0, wrd1, 109);
	}

	nr[0] = (keyBits+256)/32;

	(*env)->ReleaseByteArrayElements(env, user_key, userkey, 0);
	(*env)->ReleaseByteArrayElements(env, rk_, rk, 0);
	(*env)->ReleaseIntArrayElements(env, nr_, nr, 0);
}
...
JNIEXPORT void JNICALL
Java_com_example_android_bluetoothchat_ARIACBC_ariaDecryptInit(JNIEnv* env, jobject thiz, jbyteArray user_key, jint keyBits, jbyteArray rk_, jintArray nr_) {
    jbyte *userkey = (*env)->GetByteArrayElements(env, user_key, 0);
    jbyte *rk = (*env)->GetByteArrayElements(env, rk_, 0);
	jint *nr = (*env)->GetIntArrayElements(env, nr_, 0);
	//unsigned char rk_t[ARIA_MAXNR * (ARIA_MAXNR + 1)];

	unsigned int *rk_pint = (unsigned int *)rk;
	unsigned int *z;
	unsigned int w;
	...
	KISA_ARIA_encrypt_init(userkey, keyBits, rk, nr);
...
void internal_aria_process_blocks(unsigned char *rk, int *nr, int encrypt, unsigned char *ivec, unsigned char *in, unsigned int inl, unsigned char *out)
{
	...
	if(encrypt)
	{
		while (inlength >= ARIA_BLOCK_SIZE)
		{
			...
			KISA_ARIA_process_block(out,out, rk, nr);
		...
		while (inlength >= ARIA_BLOCK_SIZE)
		{
			KISA_ARIA_process_block(in,out, rk, nr);
		...
		if (inlength)
		{
			KISA_ARIA_process_block(in,tmp, rk, nr);
			for (i = 0; i < inlength; ++i)
				out[i] = tmp[i] ^ iv[i];
			iv = in;
		}
		...
		if (inlength)
		{
			...
			KISA_ARIA_process_block(tmp,tmp, rk, nr);
			for (i = 0; i < inlength; ++i)
				out[i] = tmp[i] ^ iv[i];
			memcpy(ivec, tmp, ARIA_BLOCK_SIZE);
		}
...
Java_com_example_android_bluetoothchat_ARIACBC_internalAriaProcessBlocks(JNIEnv* env, jobject thiz, jbyteArray rk_, jintArray nr_, jbyteArray ivec_,
		jbyteArray cbc_buffer_, jbyteArray outputText, jint outputTextLen)
{
    jbyte *rk = (*env)->GetByteArrayElements(env, rk_, 0);
	jint *nr = (*env)->GetIntArrayElements(env, nr_, 0);
    jbyte *ivec = (*env)->GetByteArrayElements(env, ivec_, 0);
    jbyte *cbc_buffer = (*env)->GetByteArrayElements(env, cbc_buffer_, 0);
    jbyte *out = (*env)->GetByteArrayElements(env, outputText, 0);

	internal_aria_process_blocks(rk, nr, 1, ivec, cbc_buffer, ARIA_BLOCK_SIZE, out + outputTextLen);

	(*env)->ReleaseByteArrayElements(env, rk_, rk, 0);
	(*env)->ReleaseIntArrayElements(env, nr_, nr, 0);
	(*env)->ReleaseByteArrayElements(env, ivec_, ivec, 0);
	(*env)->ReleaseByteArrayElements(env, cbc_buffer_, cbc_buffer, 0);
	(*env)->ReleaseByteArrayElements(env, outputText, out, 0);
}

JNIEXPORT jint JNICALL
Java_com_example_android_bluetoothchat_ARIACBC_internalAriaCBCProcessEnc(JNIEnv* env, jobject thiz, jint encrypt, jbyteArray rk_, jintArray nr_,
		jbyteArray ivec_, jbyteArray cbc_buffer_, jintArray buffer_length_, jbyteArray inputText, jint inputOffset,
		jint inLen, jbyteArray outputText, jint outputOffset)
{
	jbyte *rk = (*env)->GetByteArrayElements(env, rk_, 0);
	jint *nr = (*env)->GetIntArrayElements(env, nr_, 0);
    jbyte *ivec = (*env)->GetByteArrayElements(env, ivec_, 0);
    jbyte *cbc_buffer = (*env)->GetByteArrayElements(env, cbc_buffer_, 0);
	jint *buffer_length = (*env)->GetIntArrayElements(env, buffer_length_, 0);
    jbyte *in = (*env)->GetByteArrayElements(env, inputText, 0);
    jbyte *out = (*env)->GetByteArrayElements(env, outputText, 0);
	...
	if(buffer_length[0] == 0 && templen == 0)
	{
		internal_aria_process_blocks(rk, nr, encrypt, ivec, in, inLen, out);
		outLen=inLen;
		goto end;
	}
	...
	{
		if (buffer_length[0] + inLen < ARIA_BLOCK_SIZE)
		{
			memcpy(&(cbc_buffer[buffer_length[0]]), in, inLen);
			buffer_length[0] += inLen;
			outLen=0;
			goto end;
		}
		else
		{
			int length;
			length=ARIA_BLOCK_SIZE-buffer_length[0];
			memcpy(&(cbc_buffer[buffer_length[0]]),in,length);
			internal_aria_process_blocks(rk, nr ,encrypt,ivec,cbc_buffer,ARIA_BLOCK_SIZE,out);
	...
	(*env)->ReleaseByteArrayElements(env, ivec_, ivec, 0);
	(*env)->ReleaseByteArrayElements(env, cbc_buffer_, cbc_buffer, 0);
	(*env)->ReleaseIntArrayElements(env, buffer_length_, buffer_length, 0);
	(*env)->ReleaseByteArrayElements(env, inputText, in, 0);
	(*env)->ReleaseByteArrayElements(env, outputText, out, 0);
...
int internal_aria_cbc_process_enc(int encrypt, unsigned char *rk, int *nr, unsigned char *ivec,
		unsigned char *cbc_buffer, int *buffer_length, unsigned char *in, int inLen, unsigned char *out, int *outLen)
{
	unsigned int templen = inLen&(0x0F);
	...
	if (*buffer_length != 0)
	{
		if (*buffer_length + inLen < ARIA_BLOCK_SIZE)
		{
		...
		{
			int length;
			length=ARIA_BLOCK_SIZE-*buffer_length;
...
JNIEXPORT jint JNICALL
Java_com_example_android_bluetoothchat_ARIACBC_internalAriaCBCProcessDec(JNIEnv* env, jobject thiz, jint encrypt, jbyteArray rk_, jintArray nr_, jbyteArray ivec_,
		jbyteArray cbc_buffer_, jintArray buffer_length_, jbyteArray cbc_last_block_, jintArray last_block_flag_, jbyteArray inputText,
		jint inputOffset, jint inLen, jbyteArray outputText, jint outputOffset)
{
	jbyte *rk = (*env)->GetByteArrayElements(env, rk_, 0);
	jint *nr = (*env)->GetIntArrayElements(env, nr_, 0);
    jbyte *ivec = (*env)->GetByteArrayElements(env, ivec_, 0);
    jbyte *cbc_buffer = (*env)->GetByteArrayElements(env, cbc_buffer_, 0);
	jint *buffer_length = (*env)->GetIntArrayElements(env, buffer_length_, 0);
    jbyte *cbc_last_block = (*env)->GetByteArrayElements(env, cbc_last_block_, 0);
	jint *last_block_flag = (*env)->GetIntArrayElements(env, last_block_flag_, 0);
    jbyte *in = (*env)->GetByteArrayElements(env, inputText, 0);
    jbyte *out = (*env)->GetByteArrayElements(env, outputText, 0);
	...
	{
		memcpy(out, cbc_last_block, ARIA_BLOCK_SIZE);
		out += ARIA_BLOCK_SIZE;
		updated_len = 1;
	}
	else
	...
	internal_aria_cbc_process_enc(encrypt, rk, nr, ivec, cbc_buffer, buffer_length, in, inLen, out, &outLen);

	if (!buffer_length[0])
	{
		outLen -= ARIA_BLOCK_SIZE;
		last_block_flag[0] = 1;
		memcpy(cbc_last_block,&out[outLen],ARIA_BLOCK_SIZE);
	}
	else
		last_block_flag[0] = 0;
	...
	(*env)->ReleaseByteArrayElements(env, cbc_buffer_, cbc_buffer, 0);
	(*env)->ReleaseIntArrayElements(env, buffer_length_, buffer_length, 0);
	(*env)->ReleaseByteArrayElements(env, cbc_last_block_, cbc_last_block, 0);
	(*env)->ReleaseIntArrayElements(env, last_block_flag_, last_block_flag, 0);
	(*env)->ReleaseByteArrayElements(env, inputText, in, 0);
	(*env)->ReleaseByteArrayElements(env, outputText, out, 0);
